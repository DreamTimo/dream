...
public class DLFileOperateUtil {
	...
	private static final String DIR_THUNBNAIL = "thumbnail";
	
	public static final String DIR_ROOT = "CameraDir";
	...
	public static String getFolderPath(Context context,int type,String rootPath) {
		...
		//添加应用存储路径
		pathBuilder.append(DLFolderManager.getTempFolder().getAbsolutePath());
		...
		switch (type) {
		case TYPE_IMAGE:
			pathBuilder.append(DIR_IMAGE);
			break;
		case TYPE_VIDEO:
			pathBuilder.append(DIR_VIDEO);
			break;
		case TYPE_THUMBNAIL:
			pathBuilder.append(DIR_THUNBNAIL);
			break;
		default:
			break;
		}
	...
	public static boolean deleteThumbFile(String thumbPath,Context context) {
		...
		flag = file.delete();
		//源文件路径
		String sourcePath=thumbPath.replace(DIR_THUNBNAIL,DIR_IMAGE);
	...
	public static boolean deleteSourceFile(String sourcePath,Context context) {
		...
		flag = file.delete();
		//缩略图文件路径
		String thumbPath=sourcePath.replace(DIR_IMAGE,DIR_THUNBNAIL);
