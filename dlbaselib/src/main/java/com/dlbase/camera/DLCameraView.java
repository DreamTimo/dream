 ...
 *
 */
public class DLCameraView extends SurfaceView implements DLCameraOperation{
	...
	public DLCameraView(Context context){
		super(context);
		//初始化容器
	...
	public DLCameraView(Context context, AttributeSet attrs) {
		super(context, attrs);
		//初始化容器
		getHolder().addCallback(callback);
		openCamera();
	...
	public boolean startRecord(){
		...
		String path=DLFileOperateUtil.getFolderPath(getContext(), DLFileOperateUtil.TYPE_VIDEO, DLFileOperateUtil.DIR_ROOT);
		File directory=new File(path);
		if(!directory.exists())
			directory.mkdirs();
		try {
			String name="video"+DLFileOperateUtil.createFileNmae(".mp4");
	...
	private Bitmap saveThumbnail() throws FileNotFoundException, IOException {
		if(mRecordPath!=null){
			...
			if(bitmap!=null){
				String mThumbnailFolder=DLFileOperateUtil.getFolderPath(getContext(),  DLFileOperateUtil.TYPE_THUMBNAIL, DLFileOperateUtil.DIR_ROOT);
