...
public class DLRegexUtil {
     ...
     * @author jiqinlin
     */
    public final static boolean isRealName(String text){
        return match(text, "^[\\x{4e00}-\\x{9fa5}A-Za-z_]{2,4}$");
    }
    public final static boolean isCorrect(String text){
     ...
     * @author jiqinlin
     */
    public final static boolean isInviteCode(String text){
     ...
     * @author jiqinlin
     */
    public final static boolean isPassWord(String text){
        return match(text, "^(?!^\\d+$)(?!^[a-zA-Z]+$)[0-9a-zA-Z]{6,}$");
    }
    /**
     * 判断整数num是否等于0
     * 
    ...
    public static boolean isContainsSpecialChar(String text) {
        if(DLStringUtil.isEmpty((text))) return false;
        String[] chars={"[","`","~","!","@","#","$","%","^","&","*","(",")","+","=","|","{","}","'",
                ":",";","'",",","[","]",".","<",">","/","?","~","！","@","#","￥","%","…","&","*","（","）",
                "—","+","|","{","}","【","】","‘","；","：","”","“","’","。","，","、","？","]"};
        for(String ch : chars){
    ...
    private final static boolean match(String text, String reg) {
        if (DLStringUtil.isEmpty(text) || DLStringUtil.isEmpty(reg))
            return false;
